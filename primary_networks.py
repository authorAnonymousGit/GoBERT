...
def train_primary_model(models_df, train_df, val_df, test_df, text_feature,
    ...
    prox_mat_train = torch.tensor(utils.create_prox_mat(dist_dict_train, denominator_train, inv=False)).to(device)
    prox_mat_val = torch.tensor(utils.create_prox_mat(dist_dict_val, denominator_val, inv=False)).to(device)
    prox_mat_test = torch.tensor(utils.create_prox_mat(dist_dict_test, denominator_test, inv=False)).to(device)
    ...
    for epoch in range(epochs_num):
        ...
        train_accuracy_list.append(round(float(acc_train), 3))
...
def run_primary(task_name, model_name, train_df, val_df, test_df,
    ...
    for loss_type in loss_types:
        ...
        if loss_type == 'CrossEntropy':
            ...
            model_file_name, model_summary_rows = train_primary_model(models_df, train_df, val_df, test_df,
    ...
    with open(orig_models_path + "primary_results_summary.csv", "w+") as csv_file:
