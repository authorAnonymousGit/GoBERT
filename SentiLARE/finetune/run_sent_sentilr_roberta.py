...
def evaluate_train(args, model, tokenizer, num_labels, write_to_csv=True, prefix=""):
    # Do validation
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        # Calculate the cem on the validation set
        denominator = len(out_label_ids)
        ...
        output_eval_file = os.path.join(eval_output_dir, "train_results.txt")
        ...
        if write_to_csv:
            probabilities = list(map(lambda y: list(map(lambda x: round(x, 4), y)),
                                     softmax_values.tolist()))
            preds_new = [pred+1 for pred in preds]
            out_label_ids_new = [label+1 for label in out_label_ids]

            output_eval_file_csv = os.path.join(eval_output_dir, "train_results.csv")
...
def evaluate(args, model, tokenizer, num_labels, write_to_csv=False, prefix=""):
    # Do validation
    eval_task_names = (args.task_name,)
    eval_outputs_dirs = (args.output_dir,)
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if write_to_csv:
            ...
            output_eval_file_csv = os.path.join(eval_output_dir, "val_results.csv")
            val_results = [['key_index', 'prediction', 'probability', 'label']] + \
                           list(zip(list(range(len(preds))), preds_new, probabilities, out_label_ids_new))
...
def test(args, model, tokenizer, num_labels, prefix=""):
    # Do test
    eval_task_names = (args.task_name,)
    eval_outputs_dirs = (args.output_dir,)
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        # Calculate the cem on the test set
        denominator = len(out_label_ids)
        dist_dict = {label: 0 for label in range(1, num_labels + 1)}
        for label in out_label_ids:
            dist_dict[label + 1] += 1
        ...
        output_eval_file_csv = os.path.join(eval_output_dir, "test_results.csv")
        val_results = [['key_index', 'prediction', 'probability', 'label']] + \
                      list(zip(list(range(len(preds))), preds_new, probabilities, out_label_ids_new))
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = model_class.from_pretrained(checkpoint, pos_tag_embedding=is_pos_embedding, senti_embedding=is_senti_embedding, polarity_embedding=is_polarity_embedding)
            model.to(args.device)
            result = evaluate(args, model, tokenizer, num_labels, write_to_csv=False, prefix=global_step)
            results.extend(result)

        # Select the checkpoint with best validation accuracy
        best_valid_position = results.index(max(results))
        checkpoint_best = checkpoints[best_valid_position]
        global_step = checkpoint_best.split('-')[-1] if len(checkpoints) > 1 else ""
        model = model_class.from_pretrained(checkpoint_best, pos_tag_embedding=is_pos_embedding, senti_embedding=is_senti_embedding, polarity_embedding=is_polarity_embedding)
        model.to(args.device)
        print("*"*20)
        print("FINAL RESULTS")
        print("*"*20)
        evaluate_train(args, model, tokenizer, num_labels, write_to_csv=True, prefix="")
        evaluate(args, model, tokenizer, num_labels, write_to_csv=True, prefix=global_step)
        result = test(args, model, tokenizer, num_labels, prefix=global_step)
        logger.info("Best valid metric: %.4f" % (max(results)))
        logger.info("Test acc: %.4f" % (result[0]))
