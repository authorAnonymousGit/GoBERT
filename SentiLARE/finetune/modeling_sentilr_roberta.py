...
class RobertaForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self, config, pos_tag_embedding=False, senti_embedding=False, polarity_embedding=False):
        ...
        dist_dict = {4: 2322, 2: 2218, 3: 1624, 5: 1288, 1: 1092}
        denominator = 8544
        self.inv_prox_mat = torch.tensor(create_prox_mat(dist_dict, denominator, inv=True)).to(self.device)
        self.norm_inv_prox_mat = self.inv_prox_mat / torch.min(self.inv_prox_mat)
    ...
    def forward(self, input_ids, pos_tag_ids, senti_word_ids, polarity_ids, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                # lsm = self.logsoftmax(logits_shaped)
                # loss = CrossProxLoss6(lsm, labels.view(-1), self.inv_prox_mat, self.device)
                # print("New loss: ", loss)

                # # Loss B
                # loss = CrossProxLoss10(lsm, labels.view(-1), self.norm_inv_prox_mat, self.num_labels, 2, self.device)

                # Loss C
                # loss = CrossProxLoss10(lsm, labels.view(-1), self.norm_inv_prox_mat, self.num_labels, 4, self.device)
                
                # # Loss D
                # loss = CrossProxLoss10(lsm, labels.view(-1), self.norm_inv_prox_mat, self.num_labels ** 2, 2, self.device)
                
                # # Loss E
                # loss = CrossProxLoss10(lsm, labels.view(-1), self.norm_inv_prox_mat, self.num_labels ** 2, 4, self.device)
                
                # # Loss F
                # loss = CrossProxLoss10(lsm, labels.view(-1), self.norm_inv_prox_mat, 1, 4, self.device)
                
                # # Loss G
                # loss = CrossProxLoss10(lsm, labels.view(-1), self.norm_inv_prox_mat, self.num_labels, 5, self.device)
